...
export default function SocialModel({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        // Update social 
        response = await axiosInstance.put(`/social-data/${editId}`, formPayload);
      } else {
        // Add new social 
        response = await axiosInstance.post("/social-data/add-data ", formPayload);
      }

      const { code, message } = response.data;
      if (code === 201 || code === 200) {
        toast.success(message || "social media  successfully added/updated!");
        handleClose();
